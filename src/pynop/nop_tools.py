...
# Lazy dao: set via set_dao() from sync, or created from NOP_DB_* env on first use.
_nop_dao = None


def set_dao(dao):
    """Inject an AutoDao instance (e.g. from sync's DB_CONFIG). Call before using NopTool."""
    global _nop_dao
    _nop_dao = dao
    _load_nop_data_caches(dao)
...
def _get_nop_dao():
    ...
    if _nop_dao is None:
        ...
        _nop_dao = AutoDao(conn)
        _load_nop_data_caches(_nop_dao)
    return _nop_dao


def _load_nop_data_caches(dao):
    NopData.sku_pid_map = dao.get_hashmap(NopQuery.GET_ALL_SKU_PID_SQL)
    NopData.products_pub_del_tbl = dao.get_hashtable(NopQuery.GET_ALL_PRODUCT_PUB_DEL_SQL)
    NopData.categories = dao.get_hashtable(NopQuery.GET_ALL_CATEGORY_SQL)
    NopData.manufacturers = dao.get_hashtable(NopQuery.GET_ALL_MANUFACTURER_SQL, key=1)
    NopData.product_category_mappings = dao.get_hashtable(NopQuery.GET_ALL_PRODUCT_CATEGORY_MAPPING_SQL)
    NopData.picture_title_map = dao.get_hashmap(NopQuery.GET_ALL_PICTURE_SQL)
    NopData.product_picture_mapping_tbl = dao.get_hashtable(NopQuery.GET_ALL_PRODUCT_PICTURE_MAPPING_SQL)


class _LazyNopDao:
    __getattr__ = lambda self, name: getattr(_get_nop_dao(), name)
